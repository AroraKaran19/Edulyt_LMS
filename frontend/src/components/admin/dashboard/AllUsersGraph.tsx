...
const AllUsersGraph = () => {
    ...
    const totalCount = chartData.reduce((sum, item) => sum + item.Active + item.Unactive, 0);
                    ...
                    <div className="bg-white rounded-xl">
                        ...
                        <div className="flex gap-4">
                            <button
                                ...
                                className={`py-2 text-sm font-medium transition-colors ${selectedTab === 'all'
                                    ? 'text-[#F67124] border-b-2 font-bold text-base'
                                    : 'text-[#6B7280] hover:text-[#374151]'
                                    }`}
                            >
                                All
                                <span className="bg-[#EFF4FF] rounded-full px-2 py-1 ml-1 text-sm text-[#6B7280]">{Math.round(totalCount / 1000)}k</span>
                            </button>
                            <button
                                type="button"
                                title="Active Users"
                                onClick={() => setSelectedTab('active')}
                                className={`py-2 text-sm font-medium transition-colors ${selectedTab === 'active'
                                    ? 'text-[#F67124] border-b-2 font-bold text-base'
                                    : 'text-[#6B7280] hover:text-[#374151]'
                                    }`}
                            >
                                Active <span className="bg-[#EFF4FF] rounded-full px-2 py-1 ml-1 text-sm text-[#6B7280]">{Math.round(activeCount / 1000)}k</span>
                            </button>
                            <button
                                type="button"
                                title="Unactive Users"
                                onClick={() => setSelectedTab('unactive')}
                                className={`py-2 text-sm font-medium transition-colors ${selectedTab === 'unactive'
                                    ? 'text-[#F67124] border-b-2 font-bold text-base'
                                    : 'text-[#6B7280] hover:text-[#374151]'
                                    }`}
                            >
                                Unactive <span className="bg-[#EFF4FF] rounded-full px-2 py-1 ml-1 text-sm text-[#6B7280]">{Math.round(unactiveCount / 1000)}k</span>
                            </button>
                        </div>
                        {/* payment graph */}
                                    ...
                                    <div className="h-full min-h-72">
                                        <ResponsiveContainer width="100%" height="100%">
                                            <BarChart data={chartData} >
                                                ...
                                                />
                                                {selectedTab === 'all' && (
                                                    <Bar dataKey="Active" fill="#4323F7" barSize={20} radius={[4, 4, 4, 4]} />
                                                )}
                                                {selectedTab === 'all' && (
                                                    <Bar dataKey="Unactive" fill="#F5742C" barSize={20} radius={[4, 4, 4, 4]} />
                                                )}
                                                {selectedTab === 'active' && (
                                                    <Bar dataKey="Active" fill="#4323F7" barSize={20} radius={[4, 4, 4, 4]} />
                                                )}

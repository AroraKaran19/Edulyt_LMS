...
export const calculateCombinedDiscount = (
  ...
  // Apply plan discount first (only if active)
  if (planDiscount?.value && isDiscountActive(planDiscount)) {
  ...
  // Apply course discount on the already discounted price (only if active)
  if (courseDiscount?.value && isDiscountActive(courseDiscount)) {
...
export const calculateSimpleDiscount = (
  ...
  if (!discount?.value || !isDiscountActive(discount)) {
    return {
      discountPrice: price,
      discountLabel: "",
      discountType: "",
...
/**
 * Calculates discount information for display purposes (shows even future discounts)
 * This is used for showing discount labels and information in the UI
 * 
 * @param planPrice - The original plan price
 * @param planDiscount - Optional plan-level discount
 * @param courseDiscount - Optional course-level discount
 * @returns Discount display information
 */

/** Duplicated in Edulyt_Main/src/constants/countryCodes.ts. Behaviour must match: the two sites share a backend. */
/**
 * Keyed by ISO alpha-2, not dial code: +1 is both the US and Canada, +7 both
 * Russia and Kazakhstan, so a dial-valued <select> would pick the wrong row.
 */
...
/** Most enquiries come from India, so the field opens there. */
...
const BY_ISO = new Map(COUNTRY_CODES.map((country) => [country.iso, country]));

/** Falls back to the default, so an unknown iso cannot yield "undefined". */
export const dialFor = (iso: string): string =>
  (BY_ISO.get(iso) ?? BY_ISO.get(DEFAULT_COUNTRY_ISO))?.dial ?? "+91";

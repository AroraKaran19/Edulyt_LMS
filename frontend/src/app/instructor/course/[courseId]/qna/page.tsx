...
export default function InstructorCourseQnaPage() {
  const params = useParams();
  const searchParams = useSearchParams();
  const courseId = params.courseId as string;
  const titleFromQuery = searchParams.get("title") || "Course";
  ...
  const load = useCallback(async () => {
    ...
    try {
      const res = await apiClient.get(
        `/instructor/courses/${courseId}/qnas`,
        { params: { page, limit: 15, repliesLimit: 25 } }
      );
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-gray-600 mt-1 text-sm sm:text-base">
          <span className="font-medium text-gray-800">{titleFromQuery}</span>
          {" · "}
          Questions learners posted for this course (including pending review).
        </p>
      </div>

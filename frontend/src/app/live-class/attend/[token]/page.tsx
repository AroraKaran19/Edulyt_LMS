...
const BACK_HREF = "/dashboard/live-classes";
const BACK_LABEL = "My live classes";
...
const ERROR_COPY: Record<ErrorReason, { title: string; body: string }> = {
  invalid: {
    ...
    body: "Double-check the URL or ask your instructor to resend it.",
  },
  "not-activated": {
    title: "Not opened yet",
    body: "Your instructor hasn't opened this attendance link yet. Try again in a moment.",
  },
  expired: {
    title: "Window closed",
  ...
  "not-enrolled": {
    ...
    body: "This attendance link belongs to a course you're not enrolled in. Check you're signed in with the right account.",
  },
  "not-elite": {
    title: "Not included in your plan",
    body: "You're enrolled in this course, but live classes are part of the Elite plan. Upgrade to join live sessions and mark attendance.",
  },
  unknown: {
...
export default function LiveClassAttendPage() {
  ...
  useEffect(() => {
    ...
    if (sessionStatus === "unauthenticated") {
      const callbackUrl = `/live-class/attend/${encodeURIComponent(token)}`;
    ...
    (async () => {
      try {
        const res = await apiClient.post(ENDPOINTS.liveClasses.attend(token));
        if (cancelled) return;
        ...
        setOutcome({
          ...
          liveClassTitle: data.liveClassTitle,
          markedAt: new Date(),
        });
      } catch (err: unknown) {
        if (cancelled) return;
    ...
    <div className="max-w-xl mx-auto py-10 px-4">
      ...
      {view === "missing" ? (
        <ErrorBlock
          ...
          body="The URL is incomplete. Open the exact link your instructor shared."
        />
      ) : view === "auth" ? (
        <StatusBlock
          eyebrow="Sign in"
  ...
  return (
    <div className="border-t-2 border-amber-700 pt-6">
      ...
      <p className="mt-5 text-sm text-stone-600 leading-relaxed">
        {isFirst ? (
          <>
            Stay in the class. Your instructor will share the{" "}
      ...
      <div className="mt-8 flex items-center gap-3">
        ...
        >
          {showRetry ? (
            ...
            <>
              Back to my live classes <ArrowRight className="w-4 h-4" />
            </>
          )}
        </Link>
      </div>

...
>>>>>>> 605f3ab9c40965ad438672c34fdc5fd0fe7e7697

// Backend API base URL
const BACKEND_URL = process.env.NEXT_PUBLIC_API_URL || process.env.BACKEND_URL || 'http://localhost:5000/api'
...
async function authenticateOAuthWithBackend(
  provider: string,
  providerData: any
): Promise<{ user: BackendUser; tokens: { accessToken: string; refreshToken: string } } | null> {
  try {
...
async function getUserSession(accessToken: string): Promise<BackendUser | null> {
  try {
    ...
    async signIn({ user, account, profile }) {
      try {
        ...
        if (account?.provider !== 'credentials' && account?.provider) {
          ...
          const result = await authenticateOAuthWithBackend(account.provider, providerData)

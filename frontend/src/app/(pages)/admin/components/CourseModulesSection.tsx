...
import { Plus, X, BookOpen, GraduationCap, ChevronDown, ChevronRight, Play, FileText, Clock, CheckCircle2 } from "lucide-react";
      ...
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-gray-600 mt-1">Structure your course content into organized modules and lessons</p>
          </div>
          <button
            type="button"
            onClick={addModule}
      ...
      {modules.length === 0 ? (
        <div className="bg-white border-2 border-dashed border-gray-200 rounded-2xl p-12 text-center">
          <div className="max-w-md mx-auto">
            ...
            <p className="text-gray-600 mb-8 leading-relaxed">
              Start by creating your first module. Each module can contain multiple lessons with videos, materials, and assignments to create a comprehensive learning experience.
            </p>
        ...
        <div className="space-y-4">
          {modules.map((module, moduleIndex) => (
            <div key={module.id} className="bg-white border border-gray-200 rounded-xl shadow-sm hover:shadow-md transition-shadow duration-200">
              {/* Module Header */}
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex items-center gap-3">
                      <div className="flex items-center justify-center w-8 h-8 bg-[#F77124] rounded-lg">
                        <span className="text-white font-semibold text-sm">{moduleIndex + 1}</span>
                      </div>
                      <div>
                  ...
                  <div className="bg-gray-50 rounded-xl p-6">
                    ...
                    <div className="space-y-4">
                      {module.lessons.map((lesson, lessonIndex) => (
                        <div key={lesson.id} className="bg-white border border-gray-200 rounded-lg">
                          {/* Lesson Header */}
                                  ...
                                  {lesson.materials.map((material) => (
                                    <div key={material.id} className="flex gap-2 items-center">
                                      ...
                                      <input
                                        type="text"
                                        value={material.name}
                                        onChange={(e) => updateMaterial(module.id, lesson.id, material.id, e.target.value)}
                                        className="flex-1 px-3 py-2 text-sm border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#F77124] focus:border-transparent transition-all duration-200"
                                      ...
                                      <input
                                        ...
                                        onChange={(e) => {
                                          const file = e.target.files?.[0];
                                          if (file) handleMaterialUpload(module.id, lesson.id, material.id, file);
                                        }}
                                        className="hidden"
                              ...
                              <div className="pt-2 border-t border-gray-100">
                                <label className="flex items-center gap-2 cursor-pointer">
                                  <input
                                    ...
                                    onChange={(e) => updateLesson(module.id, lesson.id, "isForCollegeStudent", e.target.checked)}
                                    className="w-4 h-4 text-[#F77124] border-gray-300 rounded focus:ring-[#F77124] focus:ring-2"
                                  />
                                  <CheckCircle2 className="w-4 h-4 text-green-500" />
                                  <span className="text-sm text-gray-700 font-medium">Available for College Students</span>
                                </label>
                              </div>
                            </div>
                          )}

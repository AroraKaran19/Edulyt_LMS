...
import TopInternshipCard from "./TopInternshipCard";
...
const InternshipCarousel = ({
  ...
  const originalSlidesCount = internships.length;
    ...
    <div className="courses-carousel flex flex-col gap-4 items-center">
      <div className="w-full flex items-center">
        <Swiper
          ...
          initialSlide={internships.length > 3 ? 0 : 1}
          className={cn(
        ...
        >
          {internships.map((internship, index) => (
            <SwiperSlide key={`${internship.title}-${index}`} className="flex! h-auto!">
              {({ isActive }) => (
                <TopInternshipCard
          ...
          <div
            ...
            style={{
              ...
              transform: `translateX(${
                scrollProgress *
                (scrollbarWidth - 8 - parseInt(getThumbWidth()))
              }px)`,
            }}
          ></div>
        </div>

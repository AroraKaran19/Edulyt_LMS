...
import { institutePartnerDashboardJson } from "../../dummyData";
import { useState } from "react";
import { cn } from "@/lib/utils";

const data = institutePartnerDashboardJson;

const statIcons = [
  <Users key="a" className="size-5" />,
  <BookOpen key="b" className="size-5" />,
  <Percent key="c" className="size-5" />,
  <Wallet key="d" className="size-5" />,
];

export default function InstitutePartnerDashboardPage() {
    ...
    <div className="p-2 sm:p-4 py-6 space-y-4">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
        ...
        <PartnerCard className="p-4 sm:p-5">
          ...
          <div className="h-[260px] w-full">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={data.revenueBarMonthly}>
                <CartesianGrid strokeDasharray="3 3" stroke="#F2F4F7" />
                <XAxis dataKey="month" tick={{ fill: "#667085", fontSize: 12 }} />
                <YAxis tick={{ fill: "#667085", fontSize: 12 }} />
                <Tooltip />
                <Legend />
                <Bar dataKey="revenue" name="Revenue" fill="#F77124" radius={[6, 6, 0, 0]} />
                <Bar dataKey="profit" name="Profit" fill="#12B669" radius={[6, 6, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </PartnerCard>
      ...
      <PartnerCard className="p-4 sm:p-5">
        <div className="flex items-center justify-between gap-3 mb-4">
          <h2 className="text-base sm:text-xl font-semibold text-black">Courses</h2>
          <PartnerFiltersButton onClick={() => {}} />
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-sm min-w-[640px]">
            <thead>
              <tr className="text-left text-[#667085] border-b border-[#F2F4F7]">
                <th className="pb-3 font-medium">Program Name</th>
                <th className="pb-3 font-medium">Discount</th>
                <th className="pb-3 font-medium">No. of students</th>
            ...
            <tbody>
              {data.coursesTable.map((row, idx) => (
                ...
                >
                  ...
                  <td className="py-3 text-[#475467]">{row.discount}</td>

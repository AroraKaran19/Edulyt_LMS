...
const InternshipCard = ({ title, company, postedTime, application_status, applied_on, applicants, onReviewClick }: any) => (
    <div className={`flex flex-col lg:flex-row lg:items-center justify-between py-3 px-4 bg-white rounded-2xl border border-[#00000026] transition-colors gap-4 lg:gap-6`}>
        <div className="flex items-center gap-3">
            <div className="flex-1">
                <h4 className="font-normal text-lg sm:text-xl text-black font-coolvetica">{title}</h4>
                <p className="text-[#575757] font-plus-jakarta font-normal text-xs">{company}</p>
            </div>
            <div className="flex items-center gap-2 sm:gap-4">
                <div className="flex items-center gap-1 bg-[#00000078] px-2 py-1 rounded-full">
                    ...
                    <span className="text-white font-semibold font-plus-jakarta text-xs hidden sm:inline">Posted {postedTime}</span>
                    <span className="text-white font-semibold font-plus-jakarta text-xs sm:hidden">{postedTime}</span>
                </div>
            </div>
        </div>
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 sm:gap-6">
            <div className="flex items-center gap-4 sm:gap-6 w-full sm:w-auto">
                <div className="text-left flex-1 sm:flex-none">
                    <p className="text-[#475467] text-xs mb-1 font-medium">Application Status</p>
                    <p className="flex flex-row justify-start items-center gap-2">
                        <span className="text-[#5D00FF] text-lg">•</span>
                        <span className="font-bold text-black text-sm font-plus-jakarta">{application_status}</span>
                    </p>
                </div>
            ...
            >
                <span className="text-xs font-bold font-plus-jakarta text-[#656565] cursor-pointer">Review Application</span>
            </button>
        </div>
    </div>
);
                ...
                <div>
                    ...
                    <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-3 sm:gap-4">
                        <div className="flex gap-1 sm:gap-2 rounded-xl border border-[#F66F221F] bg-[#FFF6F2] p-1 sm:px-2 sm:py-1 overflow-x-auto">
                            {tabs.map((tab) => (
                                <button
                                    key={tab.label}
                                    className={`px-2 sm:px-4 py-1 sm:py-1.5 rounded-lg text-xs sm:text-sm font-medium transition-colors whitespace-nowrap ${activeTab === tab.label
                    ...
                    {internships.length === 0 ? (
                        <div className="flex flex-col items-center justify-center mt-10">
                            ...
                            <h2 className="text-xl font-bold mt-4">You Don’t Have Any Applications Yet!</h2>

...
const CurriculumSection = ({ course }: { course: Course }) => {
  ...
  return (
    <SectionContainer id="curriculum">
      <CourseTitle title="Curriculum" className="text-4xl" />
			<div className="course-information-container w-full flex gap-4 flex-col md:flex-row">
				{courseInformation.map((info) => (
					<div key={info.title} className="flex flex-col gap-2 w-full md:w-[calc((100%/3)-8px)] border-2 border-gray-100 rounded-lg p-4 items-center justify-center">
						<p className="text-lg md:text-3xl font-extrabold text-center">{info.value}</p>
						<p className="text-sm md:text-base text-center">{info.title}</p>
					</div>
				))}
			</div>
			<p className="text-sm md:text-base text-center">{course.curriculumDescription}</p>
    </SectionContainer>
  )
}

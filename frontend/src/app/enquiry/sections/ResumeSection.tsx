...
export default function ResumeSection() {
  const cms = useSection("resumes");
  const resumes = withSrc(listOr(cms.items, SAMPLE_RESUMES));
      ...
      <div className={CONTAINER}>
        <div data-reveal className={REVEAL}>
          <span className={EYEBROW}>
            ...
            {cms.eyebrow || "On your CV"}
          </span>
          <h2 className={`${H2} mt-[18px]`}>
            {cms.heading || "This is how you"}{" "}
            <em className="not-italic text-primary">
              {cms.headingHighlight || "carry them"}
            </em>
          </h2>
          <p className={LEAD}>
            {cms.lead ||
              "A badge is worth what it does on the page a recruiter actually opens. Same CV below, four different badges, so you can see where yours lands before you pick one."}
          </p>
        </div>
          ...
          <InfiniteCarousel seconds={24}>
            {resumes.map((resume) => (
              <li
                ...
                className={cn(
                  CARD,
                  "group/card w-[280px] flex-none overflow-hidden transition-[transform,box-shadow,border-color] hover:-translate-y-1.5 hover:border-[#f2d6c2]",
              ...
              >
                <div className="relative bg-[#fffcfa]">
                  ...
                  >
                    <span className="inline-flex items-center gap-1 rounded-full bg-white/90 px-2 py-1 text-[10px] font-bold text-[#8c7a70] opacity-0 shadow-[0_2px_8px_rgba(43,21,8,0.14)] transition-opacity duration-300 group-hover/card:opacity-100">

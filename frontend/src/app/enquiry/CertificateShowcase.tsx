...
import { CERTIFICATES } from "./plans";
...
export default function CertificateShowcase() {
  ...
  const shown = CERTIFICATES[active];
  ...
  return (
    <div className="mt-9 grid items-start gap-6 lg:grid-cols-[1fr_400px] lg:gap-8">
      ...
      {/* ---------- list ---------- */}
      <ul className="grid gap-1.5" role="tablist" aria-label="Certificates">
          ...
          return (
            <li key={item.src}>
              <button
                ...
                onClick={() => setActive(i)}
                className={cn(
                  "flex w-full cursor-pointer items-center gap-3 rounded-xl border px-3.5 py-2.5 text-left transition-[border-color,background-color,box-shadow] duration-200",
              ...
              >
                ...
                <ChevronRight
                  ...
                  strokeWidth={2.6}
                  className={cn(
                    "flex-none transition-[color,transform] duration-200",

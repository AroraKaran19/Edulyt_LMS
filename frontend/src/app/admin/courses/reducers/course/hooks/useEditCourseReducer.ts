...
export const useEditCourseReducer = (courseId?: string) => {
    ...
    (action: CourseAction): ReducerResult => {
      try {
        ...
        if (result.error) {
          ...
          console.error("Edit Course Reducer Error:", {
            action: action.type,
        ...
        if (result.warnings && result.warnings.length > 0) {
          console.warn("Edit Course Reducer Warnings:", {
            action: action.type,
        ...
        const runtimeError: ReducerError = {
          type: "RUNTIME_ERROR",
          message: `Unexpected error in edit course reducer: ${
        ...
        console.error("Unexpected Edit Course Reducer Error:", runtimeError);
  ...
  // Auto-save draft when state changes (only if courseId is provided)
  useEffect(() => {
  ...
  // Transform course data for edit mode - assume all media as URL source
  const transformCourseForEdit = useCallback((courseData: any) => {
    const transformedCourse = { ...courseData };

    // Transform course-level media to URL source
    if (transformedCourse.thumbnail) {
      transformedCourse.thumbnailUrl = transformedCourse.thumbnail;
      transformedCourse.thumbnailSource = "url";
      transformedCourse.thumbnailS3Key = "";
    }
    if (transformedCourse.previewVideoUrl) {
      transformedCourse.previewVideoSource = "url";
      transformedCourse.previewVideoS3Key = "";
    }

    // Transform module media
    if (transformedCourse.modules) {
      transformedCourse.modules = transformedCourse.modules.map((module: any) => {
        const transformedModule = { ...module };
        
        if (transformedModule.thumbnailUrl) {
          transformedModule.thumbnailSource = "url";
          transformedModule.thumbnailS3Key = "";
        }

        // Transform lesson content media
        if (transformedModule.lessons) {
          transformedModule.lessons = transformedModule.lessons.map((lesson: any) => {
            const transformedLesson = { ...lesson };

            if (transformedLesson.contents) {
              transformedLesson.contents = transformedLesson.contents.map((content: any) => {
                const transformedContent = { ...content };

                // Transform video content media
                if (content.type === 'video') {
                  // Transform video sources
                  if (transformedContent.sources) {
                    transformedContent.sources = transformedContent.sources.map((source: any) => ({
                      ...source,
                      videoSource: "url",
                      videoS3Key: ""
                    }));
                  ...
                  // Transform video thumbnail
                  if (transformedContent.thumbnailUrl) {
                    transformedContent.thumbnailSource = "url";
                    transformedContent.thumbnailS3Key = "";
                  }
                }
  ...
  const actions = {
    ...
    setCourse: useCallback(
      (course: any) => {
        // Transform course data for edit mode
        const transformedCourse = transformCourseForEdit(course);
        const action = courseActions.setCourse(transformedCourse);
        return enhancedDispatch(action);
      },
      [enhancedDispatch, transformCourseForEdit]
    ),
      ...
      (courseId: string) => {
        ...
        if (editDraftUtils.hasDraft() && editDraftUtils.isDraftForCourse(courseId)) {
          ...
          if (draft) {
            ...
            const transformedDraft = transformCourseForEdit(draft);
            const action = courseActions.setCourse(transformedDraft);
            return enhancedDispatch(action);
          }
        }
        return { state, error: undefined, warnings: ["No edit draft found"] };
      },
      [enhancedDispatch, state, transformCourseForEdit]
    ),
    ...
    setCourseThumbnailSource: useCallback(
      (thumbnailSource: "upload" | "url" | undefined) => {
        const action = courseActions.setCourseThumbnailSource(thumbnailSource);
        return enhancedDispatch(action);
      },
      [enhancedDispatch]
    ),
    ...
    setCourseThumbnailS3Key: useCallback(
      (thumbnailS3Key: string) => {
        const action = courseActions.setCourseThumbnailS3Key(thumbnailS3Key);
        return enhancedDispatch(action);
      },
      [enhancedDispatch]
    ),
    ...
    setCoursePreviewVideoSource: useCallback(
      (previewVideoSource: "upload" | "url" | undefined) => {
        const action = courseActions.setCoursePreviewVideoSource(previewVideoSource);
        return enhancedDispatch(action);
      },
      [enhancedDispatch]
    ),
    ...
    setCoursePreviewVideoS3Key: useCallback(
      (previewVideoS3Key: string) => {
        const action = courseActions.setCoursePreviewVideoS3Key(previewVideoS3Key);
        return enhancedDispatch(action);
      },
      [enhancedDispatch]
    ),

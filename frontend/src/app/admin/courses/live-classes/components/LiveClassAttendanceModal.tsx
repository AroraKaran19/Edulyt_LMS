...
function verdictBadge(verdict: LiveClassVerdict, overridden: boolean) {
  const base =
...
export default function LiveClassAttendanceModal({
  liveClassId,
  onClose,
}: Props) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setData(res.data?.data as LiveClassAttendanceResponse);
    } catch {
  ...
  const handleOverride = async (
    ...
    if (!liveClassId || busyUserId) return;
    ...
    try {
      ...
      // The API returns the whole refreshed roster, so the counts and every
      // verdict stay consistent without a second request.
      setData(res.data?.data as LiveClassAttendanceResponse);
      toast.success(
        verdict === "clear"
      ...
      const msg =
        (e as { response?: { data?: { error?: { message?: string } } } })
          ?.response?.data?.error?.message ?? "Failed to update attendance";
      toast.error(msg);
    } finally {
      setBusyUserId(null);
    }
  ...
  const isOpen = liveClassId !== null;
  const presentCount =
    data?.rows.filter((r) => r.verdict === "present").length ?? 0;
  const absentCount =
    data?.rows.filter((r) => r.verdict === "absent").length ?? 0;
  const pendingCount =
    data?.rows.filter((r) => r.verdict === "pending").length ?? 0;
        ...
        <div className="flex flex-col gap-4">
          <div className="flex flex-wrap items-center gap-4 text-sm">
            <div className="min-w-0">
              ...
              <span className="text-gray-500 ml-2">
                {formatIstDateTime(data.liveClass.startDateTime)}
              </span>
              {data.liveClass.course?.title ? (
          ...
          {pendingCount > 0 && (
            <p className="text-xs text-gray-500 -mt-2">
              Verdicts stay pending until both attendance windows have closed.
              Absences are only recorded once they do.
            </p>
          )}
              ...
              <tbody className="divide-y divide-gray-200">
                {data.rows.length === 0 ? (
                  <tr>
                    ...
                    >
                      No elite-plan learners enrolled in this course yet.
                    </td>
                  </tr>
                ) : (
                  data.rows.map((row) => (
...
function AttendanceRow({
  ...
  row: LiveClassAttendanceRow;
  busy: boolean;
  disabled: boolean;
  onOverride: (userId: string, verdict: "present" | "absent" | "clear") => void;
}) {

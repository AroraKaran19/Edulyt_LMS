    ...
    try {
      ...
      const msg =
        (err as { response?: { data?: { error?: { message?: string } } } })
          ?.response?.data?.error?.message ?? "Could not update meeting URL";
      toast.error(msg);
    } finally {
      setSubmitting(false);
    }
    ...
    >
      <div className="flex flex-col gap-4">
        <p className="text-xs text-gray-500">
          Only the meeting URL can be edited. To change anything else, delete
          this meeting and create a new one.
        </p>
        <Input
          label="Meeting URL"
          required

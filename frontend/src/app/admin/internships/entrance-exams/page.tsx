...
import type { EntranceExamCohortRow } from "@/types";
...
export default function EntranceExamsAdminPage() {
  ...
  const [rows, setRows] = useState<EntranceExamCohortRow[]>([]);
  ...
  const load = useCallback(async () => {
    ...
    try {
      const res = await apiClient.get(
        ENDPOINTS.internshipEnrollments.adminEntranceExamCohorts,
      );
      ...
      toast.error("Failed to load entrance exam cohorts");
      setRows([]);
    } finally {
      setLoading(false);
    }
  ...
  const goToCohort = (r: EntranceExamCohortRow) => {
    const q = new URLSearchParams({
      internshipId: r.internshipId,
      batchId: r.batchId,
    });
    router.push(`/admin/internships/entrance-exams/cohort?${q.toString()}`);
  };
    ...
    <div className="p-4 sm:p-6">
      <div className="mb-6">
        ...
        <p className="text-gray-600 mt-1">
          Cohorts with an entrance exam — open a row to manage registrations
          and submissions
        </p>
      </div>
                  ...
                  <td colSpan={6} className="px-6 py-12 text-center">
                    <div className="flex flex-col items-center gap-2">
                      ...
                      <p className="text-gray-500 font-medium">
                        No cohorts with an entrance exam
                      </p>
                      <p className="text-gray-400 text-sm max-w-md">
                        Link an entrance exam to a batch in Manage internships.
                      </p>
                    </div>
                  </td>
                </tr>

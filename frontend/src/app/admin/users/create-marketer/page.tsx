  ...
  const submit = async () => {
    ...
    try {
      await apiClient.post("/admin/staff/admins", {
        ...
        password,
        role: "marketer",
        // The server forces a marketer's permissions empty regardless, since
        // the role is the grant.
        permissions: [],
      });
      toast.success(`${firstName.trim()} can now sign in as a marketer`);
      router.push("/admin/users/manage-users");
    } catch (error) {
      toast.error(errorMessage(error, "Could not create the marketer"));
    } finally {
      setSubmitting(false);
    }
  };
      ...
      <div className="mb-6 flex items-start gap-3">
        ...
        <div>
          ...
          <p className="text-gray-600 mt-1">
            A marketer can open Scholarship campaigns and nothing else, and only
            sees campaigns they created themselves.
          </p>
        </div>
      </div>
      ...
      <div className="flex flex-col gap-6">
        ...
        <section className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 flex flex-col gap-4">
          ...
          <div className="rounded-xl border border-gray-200 bg-gray-50/60 p-4">
            <p className="text-sm text-gray-700">
              Share these credentials with the marketer directly. They can change
              the password from their profile, subject to the usual 7-day change
              cooldown.
            </p>
          </div>
        </section>
        ...
        <div className="flex justify-end gap-2">
          ...
          <OrangeButton
            ...
            onClick={() => void submit()}
          >
            {submitting ? "Creating…" : "Create marketer"}
          </OrangeButton>
        </div>
      </div>
    </div>

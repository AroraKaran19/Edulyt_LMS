...
const CertificateJobsPage = () => {
  ...
  const loadJobs = useCallback(async () => {
    ...
    try {
      ...
      console.error("Failed to fetch certificate jobs:", err);
      toast.error("Failed to load certificate jobs");
      setJobs([]);
    } finally {
      setIsLoading(false);
    }
  ...
  const handleRetry = async (jobId: string) => {
    ...
    try {
      await apiClient.post(`/admin/certificate-jobs/${jobId}/retry`);
                  ...
                  <th className="relative overflow-visible text-left py-4 px-4 font-semibold text-gray-700">
                    <span className="group/tooltip relative inline-flex cursor-help items-center gap-1.5">
                      ...
                      <span className="pointer-events-none absolute left-0 top-full z-50 mt-1.5 w-64 rounded-lg bg-gray-900 px-3 py-2 text-xs font-normal text-white shadow-lg opacity-0 transition-opacity duration-150 group-hover/tooltip:opacity-100">
                        Certificate generation stages: 10% started → 20%
                        enrollment → 30% user/course → 40% validated → 50–90%
                        PDF generation → 100% completed
                      </span>
                    </span>
                  </th>
                  <th className="text-left py-4 px-4 font-semibold text-gray-700">
                    ...
                    >
                      ...
                      <td className="py-3 px-4 text-right">
                        <div className="flex items-center justify-end gap-2">
                          {job.status === "completed" && job.certificateUrl && (
                            <a
...
export default CertificateJobsPage;

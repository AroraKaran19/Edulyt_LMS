...
/**
 * Grant free enrollments in every active course that shares the purchased
 * course's PRIMARY category (`category[0]`). Called after a paid purchase so a
 * learner who buys one course in a domain unlocks its siblings for free.
 *
 * Gated behind CATEGORY_SIBLING_ENROLLMENT_ENABLED (must equal the string
 * "true"; unset/anything-else = disabled). Read per-call so the flag can be
 * flipped without redeploying running workers.
 *
 * Granted enrollments are ordinary, fully-valid enrollments (progress,
 * certificates, My Courses, analytics all work). They carry `grantSource` and
 * `grantedFromCourseId` as traceability markers only.
 *
 * Best-effort: callers MUST wrap this so a failure never fails order fulfilment.
 */
export const grantCategorySiblingEnrollments = async (params: {
  userId: string;
  /** The paid course, already loaded by the caller (populated or lean). */
  purchasedCourse: {
    _id: any;
    category?: any[];
  };
  ...
  if (process.env.CATEGORY_SIBLING_ENROLLMENT_ENABLED !== "true") {
    return;
  }

  const { userId, purchasedCourse, planType } = params;
  const purchasedCourseId = String(purchasedCourse._id);

  // Primary category = first entry of the course's category array.
  const primaryCategoryId = purchasedCourse.category?.[0];
  if (!primaryCategoryId) {
    return;
  }

  // Sibling courses: active, in the same primary category, excluding the one
  // just purchased. `category: id` matches any course whose category array
  // contains that id (multikey index on `category`).
  const siblings = await CourseModel.find({
    _id: { $ne: purchasedCourseId },
  ...
  // Subtract courses the user already holds. Status-agnostic ON PURPOSE: the
  // unique { userId, courseId } index has no partial filter, so even a
  // revoked/dropped enrollment would collide on insert.
  const existing = await EnrollmentModel.find({
    ...
    .select("courseId")
    .lean();
  const alreadyEnrolled = new Set(existing.map((e) => String(e.courseId)));

  const toGrant = siblings.filter((c) => !alreadyEnrolled.has(String(c._id)));
  if (toGrant.length === 0) {
    return;
  }

  const now = new Date();
  // insertMany bypasses the pre("save") hook, so set validUntil explicitly.
  // Without it, checkEnrollment's "no expiry set -> valid" branch would treat
  // these as never-expiring. Give each grant its own fresh 4-year window.
  ...
  const docs = toGrant.map((c) => ({
    ...
    totalTimeSpent: 0,
    validUntil,
    grantSource: "category-sibling" as const,
    grantedFromCourseId: purchasedCourseId,
  }));

  // Insert grants. `ordered: false` lets non-duplicate inserts land even if a
  // concurrent purchase already created one of them; duplicate-key (E11000)
  // errors are treated as success, anything else rethrown to the caller.
  let insertedDocs: { _id: any; courseId: any }[] = [];
  try {
    ...
    const isDuplicateKey =
      ...
      (Array.isArray(err?.writeErrors) &&
        err.writeErrors.every(
          (we: any) => we?.err?.code === 11000 || we?.code === 11000
        ));
    if (!isDuplicateKey) {
      throw err;
    }
    // Partial success: only the docs Mongo actually inserted are on err.insertedDocs.
    insertedDocs = (err?.insertedDocs ?? [])
      .filter((d: any) => d?._id)
      .map((d: any) => ({ _id: d._id, courseId: d.courseId }));
  }
  ...
  // Link grants to the student and bump analytics for ONLY the granted courses.
  const insertedEnrollmentIds = insertedDocs.map((d) => d._id);
  const grantedCourseIds = insertedDocs.map((d) => d.courseId);

  await Promise.all([
    StudentModel.findByIdAndUpdate(userId, {
      $push: { enrollments: { $each: insertedEnrollmentIds } },
    }),
    CourseModel.updateMany(
      { _id: { $in: grantedCourseIds } },

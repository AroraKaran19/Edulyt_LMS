...
const affiliateSchema = new mongoose.Schema<Affiliate>({
  code: {
    ...
    validate: {
      ...
      message: 'Affiliate code must contain only uppercase letters, numbers, hyphens, and underscores'
    }
  },
  createdBy: {
...
affiliateSchema.index({ code: 1 }, { unique: true }); // Unique affiliate codes
affiliateSchema.index({ createdBy: 1 }); // Find affiliates by creator
affiliateSchema.index({ totalReferrals: -1 }); // Sort by referral count
affiliateSchema.index({ createdAt: -1 }); // Recent affiliates
...
// Instance methods
affiliateSchema.methods.addReferral = function(userId: string) {
...
affiliateSchema.methods.removeReferral = function(userId: string) {
...
export const AffiliateModel = mongoose.model<Affiliate>("Affiliate", affiliateSchema);
